...
export function BotanicalsGrid({ ingredients }: { ingredients: Ingredient[] }) {
  ...
  const filtered = useMemo(
    () =>
      group === "All"
        ? ingredients
        : ingredients.filter((ingredient) => ingredient.group === group),
    [ingredients, group]
  );

  return (
    <div>
      <div className="flex flex-wrap gap-2" role="tablist" aria-label="Botanical groups">
        {GROUPS.map((g) => (
          <button
            key={g}
            type="button"
      ...
      <div className="mt-kb-8 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
